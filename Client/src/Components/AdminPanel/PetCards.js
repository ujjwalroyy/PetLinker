...
const PetCards = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await fetch(`https://pet-linker-ug8m.vercel.app/approving/${props.pet._id}`, {
        method: 'PUT',
        body: JSON.stringify({
          status: "Approved"
        }),
  ...
  const deleteFormsAdoptedPet = async () => {
    setIsDeleting(true)
    try {
      const deleteResponses = await fetch(`https://pet-linker-ug8m.vercel.app/form/delete/many/${props.pet._id}`, {
        method: 'DELETE'
      });
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(`https://pet-linker-ug8m.vercel.app/delete/${props.pet._id}`, {
        method: 'DELETE'
      })
    ...
    <div className='req-containter'>
      <div className='pet-view-card'>
        <div className='pet-card-pic'>
          <img src={`https://pet-linker-ug8m.vercel.app/images/${props.pet.filename}`} alt={props.pet.name} />
        </div>
